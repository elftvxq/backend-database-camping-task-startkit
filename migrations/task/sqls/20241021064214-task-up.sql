...
((SELECT id FROM "USER" WHERE name = '王小明'), 
 (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案'), 14, 
 (SELECT price FROM "CREDIT_PACKAGE" WHERE name = '14 堂組合包方案')),

((SELECT id FROM "USER" WHERE name = '王小明'), 
 (SELECT id FROM "CREDIT_PACKAGE" WHERE name = '21 堂組合包方案'), 21, 
...
FROM "COACH" c, "SKILL" s
WHERE s.name = '瑜伽'
AND c.user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io');
...
FROM "COACH" c, "SKILL" s
WHERE s.name = '有氧運動'
AND c.user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io');
...
VALUES (
    (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io'),  -- 李燕容的 user_id
    (SELECT id FROM "SKILL" WHERE name = '重訓'),  -- 重訓的 skill_id
    '重訓基礎課',  -- 課程名稱
...
VALUES (
    (SELECT id FROM "USER" WHERE name = '王小明'),  -- 王小明的 user_id
    (SELECT id FROM "COURSE" WHERE name = '重訓基礎課' AND user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')),  -- 李燕容的課程 id
...
INSERT INTO "COURSE_BOOKING" (user_id, course_id, booking_at, status)
VALUES (
    (SELECT id FROM "USER" WHERE name = '好野人'),  -- 好野人的 user_id
    (SELECT id FROM "COURSE" WHERE name = '重訓基礎課' AND user_id = (SELECT id FROM "USER" WHERE email = 'lee2000@hexschooltest.io')),  -- 李燕容的課程 id
    '2024-11-24 16:00:00',  -- 預約時間
    '即將授課'  -- 狀態
);
...
SET cancelled_at = '2024-11-24 17:00:00',  -- 取消預約時間
    status = '課程已取消'  -- 狀態
...
SELECT u.id AS user_id, COUNT(cb.id) AS total
FROM "COURSE_BOOKING" cb
JOIN "USER" u ON cb.user_id = u.id
WHERE u.name = '王小明'
AND cb.status = '上課中'  -- 假設只有狀態為 "上課中" 的記錄算作已使用
GROUP BY u.id;
...
SELECT u.id AS user_id, 
       (COALESCE(SUM(cp.purchased_credits), 0) - COALESCE(COUNT(cb.id), 0)) AS remaining_credit
FROM "USER" u
LEFT JOIN "CREDIT_PURCHASE" cp ON u.id = cp.user_id
LEFT JOIN "COURSE_BOOKING" cb ON u.id = cb.user_id AND cb.status = '上課中'
WHERE u.name = '王小明'
GROUP BY u.id;
...
SELECT cp.name AS 組合包方案名稱, COUNT(cpr.id) AS 銷售數量
FROM "CREDIT_PURCHASE" cpr
INNER JOIN "CREDIT_PACKAGE" cp ON cpr.credit_package_id = cp.id
WHERE EXTRACT(MONTH FROM cpr.purchase_at) = 11
GROUP BY cp.name;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收

SELECT SUM(cpr.price_paid) AS 總營收
FROM "CREDIT_PURCHASE" cpr
WHERE EXTRACT(MONTH FROM cpr.purchase_at) = 11;
